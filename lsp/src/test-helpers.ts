...
export type PartialDefInput = Omit<PartialDef, 'slots' | 'freeVars' | 'dataShape' | 'customElement'> & { slots?: string[]; freeVars?: string[]; dataShape?: Map<string, DataShape>; customElement?: boolean };
...
export function makeIndex(defs: PartialDefInput[], refs: PartialRefInput[]): ProjectIndex {
	...
	for (const d of defs) {
		const def: PartialDef = { ...d, slots: d.slots ?? [], freeVars: d.freeVars ?? [], dataShape: d.dataShape, customElement: d.customElement ?? false };
