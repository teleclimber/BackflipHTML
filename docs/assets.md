...
- **Language Server (LSP)**: The VSCode extension provides auto-completion for asset paths, hover previews (with image dimensions), go-to-definition, find-all-references, and red squiggles for missing files. It also provides an Asset Report panel. Find-all-references lists every use of an asset across your templates *and* the stylesheets in your asset directories.
...
- **dom-patch script auto-include**: For the renderer to auto-include the JS generated by a [`dom-patch` output](configuration.md#lang-dom-patch), an asset entry must cover that output directory (its `path` equals or contains the output dir). The script's public URL is then the asset `prefix` followed by the file's path relative to the asset dir; the most specific asset dir wins when several cover the file. If nothing covers it, the build warns and the scripts are not auto-included.
